...
//    all (see SampleData.h's own Channel/Aux distinction), so carried as
...
public:
  AudioBlockEvent(SampleData master, SampleData raw_bus, SampleData aux_a, SampleData aux_b)
  ...
  const SampleData & getAuxA() const { return aux_a_; }
  const SampleData & getAuxB() const { return aux_b_; }
