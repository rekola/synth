...
**Known prerequisite (from the FFTW→PocketFFT migration plan): re-verify cross-thread FFT contention before shipping.** By the time this feature lands, the engine's FFT backend is expected to be PocketFFT rather than FFTW (see `plans/magical-wondering-engelbart.md`) — PocketFFT has no persistent plan object; every transform call re-touches an internal, mutex-guarded twiddle-factor cache instead. That migration deliberately converts the audio-thread spectrum analyzer (`Player.cpp`'s FFT chart) to PocketFFT and accepts the resulting uncontended-mutex risk, on the reasoning that nothing else touches PocketFFT's cache concurrently *at that time*. `DiracAnalyzer` breaks that assumption: it runs its own `RealFFT`-based STFT on the UI thread, genuinely concurrently with the audio thread's own FFT calls, for the first time in this codebase. Before this feature ships, re-check (measure, don't assume) whether that real concurrent contention is actually benign in practice (uncontended-lock cost vs. any observed priority-inversion/glitching under load) — the FFTW→PocketFFT plan explicitly deferred this exact question to here rather than resolving it in the abstract.
