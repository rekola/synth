...
// AmbisonicStereoMixer/AmbisonicBinauralMixer - there is no separate plain-
// stereo-pan mixer anymore (BasicMixer was retired). A MONO (0th-order-
// ambisonic, W-only) config never attempts binaural decoding - there's no
// directional content to convolve - so it always falls straight through to
// the cardioid AmbisonicStereoMixer below, same as the AMBISONIC fallback
// path when no SOFA file resolves.
unique_ptr<Mixer>
createMixer(const ChannelConfiguration & channel_config, MixerType mixer_type) {
  ...
  auto channels = channel_config.numberOfChannels(); // 1 (MONO), or 4/9 (AMBISONIC)
