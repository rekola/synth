...
class GenericInstrument : public Instrument {
 public:
  ...
  std::unique_ptr<TrackState> playNote(const ChannelConfiguration & channel_config, float azimuth, float frequency, float detune, float velocity, float start_phase, int note_value) const override {
    ...
    auto voice = concrete_instrument_->playNote(channel_config, azimuth, frequency, detune, velocity, start_phase, note_value);

    // don't pass velocity or azimuth to children
    for (auto & child : getChildren()) {
      auto modulator = child->playNote(channel_config, 0.0f, frequency, detune, 1.0, start_phase, note_value);
