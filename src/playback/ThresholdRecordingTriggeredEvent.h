...
// order, ready to prepend as-is), and `section`/`row` are the transport's
...
public:
  ThresholdRecordingTriggeredEvent(int track_id, AudioBuffer preroll, int section, int row)
    : track_id_(track_id), preroll_(std::move(preroll)), section_(section), row_(row) { }
