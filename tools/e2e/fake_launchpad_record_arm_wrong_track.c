// Regression test for a bug where handlePadEvent()'s own recording-
// supersedes-assigned-track override only fired when the recording
// track's internal id happened to sort numerically lower than the
// assigned/cursor track's - track "1" here (index 1, armed/targeted for
// recording) is created after, and so has a higher internal id than,
// track "0" (index 0, the default assigned/cursor track, left untouched
// this whole script) - exactly the common case (a track armed/selected
// later in a session) the bug missed. Arms and targets track 1's clip
// index 0, then plays a NOTE-mode note while track 0 is still the
// assigned/cursor track - a correct implementation routes the note (and
// records it) onto track 1, not track 0.
...
int main() {
  ...
  if (port < 0) return 1;
  fprintf(stderr, "fake Launchpad X (record arm wrong track) ready as client %d port %d\n", snd_seq_client_id(seq), port);
  ...
  fprintf(stderr, "sending press on pad (1,0) [note 12] - picker row, column 1 - arms track 1, NOT track 0\n");
  send_note(seq, port, 0x90, 12, 100);
  send_note(seq, port, 0x80, 12, 0);
  drain(seq, 500, "track 1 armed");
  ...
  fprintf(stderr, "sending press+release on pad (0,0) [note 11] in Note mode - should route to track 1, not track 0\n");
  ...
  fprintf(stderr, "sending press on pad (1,0) [note 12] again - disarms track 1, finalizing the take\n");
  send_note(seq, port, 0x90, 12, 100);
  send_note(seq, port, 0x80, 12, 0);
  drain(seq, 1000, "track 1 disarmed");
