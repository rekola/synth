...
// the recording-supersedes-assigned-track override, so a NOTE-mode press
// toggled a step in the *background* pattern instead of ever reaching
// the armed take at all. Arms and targets the fixture's only track
// (clip index 0), plays a NOTE-mode pad press, then disarms - a correct
// implementation routes that press through ordinary free-drumming
// percussion note entry (resolveNote()'s own percussion branch) into the
// armed clip, not the step grid.
...
int main() {
  ...
  if (port < 0) return 1;
  fprintf(stderr, "fake Launchpad X (record arm percussion) ready as client %d port %d\n", snd_seq_client_id(seq), port);
  ...
  drain(seq, 500, "recording armed at clip index 0");
  ...
  fprintf(stderr, "sending press+release on pad (0,0) [note 11] - must reach the armed take, not toggle a step\n");
  send_note(seq, port, 0x90, 11, 100);
  send_note(seq, port, 0x80, 11, 0);
