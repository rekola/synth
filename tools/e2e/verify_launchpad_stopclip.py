"""Regression test for the redesigned Stop Clip (CC49): a held modifier,
not a plain single press targeting an invisible "current track" - holding
it and pressing any pad in a track's column stops that track
(LaunchpadManager::handleStopClipButton()/handleSessionPadEvent()).
Triggers pool index 7 for the fixture's only track via pad (0,0),
confirms its own LED (note 11 / led index 0x0b) brightens, then holds
CC49 and presses that same pad again to queue a stop, and confirms the
LED reverts once the quantized stop actually takes effect (the fixture's
pooled pattern has no explicit length, so the stop resolves on the very
next audition-clock step - see triggerPooledPatternStep()'s own
current_length <= 0 -> 1 fallback)."""
