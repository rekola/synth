...
public:
  OscilatorVoice(unsigned int _outSampleRate, WaveformType _type, int _harmonic, int _subharmonic, float _level)
    : InstrumentVoice(_outSampleRate), type(_type), harmonic(_harmonic), subharmonic(_subharmonic), level(_level) {
  }
...
Oscilator::playNote(float frequency, float velocity, unsigned int outSampleRate, float start_phase) const {  
  auto voice = std::make_unique<OscilatorVoice>(outSampleRate, type, harmonic, subharmonic, level);
  voice->playNote(frequency, velocity, start_phase);

  // don't pass velocity to children
  for (auto & child : getChildren()) {
    auto modulator = child->playNote(frequency, 1.0, outSampleRate, start_phase);
