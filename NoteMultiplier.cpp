...
NoteMultiplier::playNote(const ChannelConfiguration & channel_config, float azimuth, float frequency, float velocity, float start_phase) const {
  ...
  for (auto & child : getChildren()) {
    ...
    // root
    auto voice = child->playNote(channel_config, azimuth, frequency, unison_velocity, start_phase + getRandF());
    if (voice.get()) group->addChild(move(voice));

    float frequency_start = frequency * (1 - detune_ / 2);
    float frequency_end = frequency * (1 + detune_ / 2);

    float azimuth_start = azimuth - spread_ / 2;

    if (unisons_ >= 2) {
      float frequency_step = powf(frequency_end / frequency_start, 1.0f / (unisons_ - 1));
      float azimuth_step = spread_ / (unisons_ - 1);
      
      // unisons_
      for (int i = 0; i < unisons_; i++) {
	auto voice = child->playNote(channel_config, azimuth_start + i * azimuth_step, frequency_start * powf(frequency_step, i), unison_velocity, start_phase + getRandF());
	if (voice.get()) group->addChild(move(voice));
      }
    }
    ...
    for (i = 0, m = 2; i < octaves_; i++, m *= 2) {
      float f = frequency;
      if (detune_ != 0) f *= getRandF() * (detune_ - 0.5 * detune_);
      auto voice = child->playNote(channel_config, azimuth, f * m, velocity / m, start_phase + getRandF());
      if (voice.get()) group->addChild(move(voice));
    }
